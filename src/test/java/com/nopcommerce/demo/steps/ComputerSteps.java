...
public class ComputerSteps {
    ...
    public void verifyTheTextBuildYourOwnComputer() {
        org.junit.Assert.assertEquals("Build your own computer message is not displayed", "Build your own computer", new BuildYourOwnComputerPage().getBuildYourOwnComputerText());
    }
    ...
    public void verifyThePrice(String expPrice) throws InterruptedException {
        Thread.sleep(2000);
        org.junit.Assert.assertEquals("$1,475.00 is not displayed", expPrice, new BuildYourOwnComputerPage().getPrice());
    }
    ...
    public void verifyTheMessageOnTopGreenBar(String expMess) {
        org.junit.Assert.assertEquals("Product added message is not displayed", expMess, new BuildYourOwnComputerPage().getProductAddedText());
    }
    ...
    public void iCanVerifyTheMessage(String expMess) {
        org.junit.Assert.assertEquals("Shopping cart is not displayed", expMess, new CartPage().getShoppingCartText());
    }
    ...
    public void verifyTheTotal(String expTotal) {
        org.junit.Assert.assertEquals("$2,950.00 is not displayed", expTotal, new CartPage().getTotal());
    }
    ...
    public void verifyTheText(String expMess) {
        org.junit.Assert.assertEquals("Welcome message is not displayed", expMess, new CartPage().getWelcomeText());
    }
    ...
    public void verifyPaymentMethodIs(String card) {
        org.junit.Assert.assertEquals("Credit card text is not displayed", card, new ConfirmOrderPage().getCreditCardText());
    }

    @And("^Verify shipping method is \"([^\"]*)\"$")
    public void verifyShippingMethodIs(String nextDay) {
        org.junit.Assert.assertEquals("Next day air shipping method is not displayed",nextDay,new ConfirmOrderPage().getNextDayAirText());
    }
    ...
    public void iCanVerifyText(String text) {
        org.junit.Assert.assertEquals("Thank you text is not displayed",text,new ConfirmOrderPage().getThankYouText());
    }

    @And("^Verify the message \"([^\"]*)\"$")
    public void verifyTheMessage(String text) {
        org.junit.Assert.assertEquals("Success message is not displayed",text,new ConfirmOrderPage().getSuccessfullyMessage());
    }
